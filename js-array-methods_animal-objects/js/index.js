...
const indexOfAnimalWithNameLongerFive = animals.map((animal) => {
...
const animalsSortedAlphabetically = animals.toSorted((a, b) => a.name.localeCompare(b.name));
...
const animalWithWeightMoreThanFivehundredExists = animals.find(animal => animal.weight > 400);
...
const averageWeightOfAllAnimalsInAfrica = animals.filter(animal => {
  ...
  .reduce(
    (accumulator, weight) => (accumulator + weight / animals.filter(animal => animal.continents.includes("Africa")).length), 0
  );
